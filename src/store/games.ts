...
export const useGameStore = defineStore({
  ...
  actions: {
    initialise() {
      if (localStorage.getItem("gn-tools:games")) {
        this.games = JSON.parse(
          localStorage.getItem("gn-tools:games") as string
        );
      }
    },
    ...
    removeGameById(id: string) {
      ...
      this.games.splice(
        this.games.findIndex((listedGame: Game) => listedGame.id === id),
        1
      );
      localStorage.setItem("gn-tools:games", JSON.stringify(this.games));
  ...
  getters: {
    ...
    byId: (state: GameState) => (id: string) =>
      state.games.find((game: Game) => game.id === id)
  }
});
